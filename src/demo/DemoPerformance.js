...
export class DemoPerformance extends BaseComponent {
    ...
    updatePerformanceMetrics() {
        ...
        const avgMountTime = this.performanceData.mountTimes.length > 0
            ? Math.round(this.performanceData.mountTimes.reduce((a, b) => a + b, 0) / this.performanceData.mountTimes.length)
            : 0;
        ...
        const avgNavTime = this.performanceData.navigationTimes.length > 0
            ? Math.round(this.performanceData.navigationTimes.reduce((a, b) => a + b, 0) / this.performanceData.navigationTimes.length)
            : 0;
    ...
    updateHealthStatus() {
        ...
        if (componentsStatus) {
            componentsStatus.textContent = componentsHealth === 'healthy' ? 'All OK' : `${componentErrors} Errors`;
        }
