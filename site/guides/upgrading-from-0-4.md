...
0.5 is a breaking release. Renamed attributes and events kept working through 0.5, but some changes need updates to your build, styles, markup or server code. Work through the sections that apply to your site, then follow [Upgrading from 0.5 to 0.6](upgrading.html), which removes the deprecated names.
...
Import paths work with or without `.js`. Use the normal paths rather than the old `@parallelogram-js/core/dev/*` paths, which 0.6.0 removes. Bundlers that resolve the `development` export condition, as Vite does during development, pick the development build.
